...
function interestedClick() {
    if (selectedActivity) {
        const selectedActivityCopy = { ...selectedActivity } // spread so we don't mutate the activity.. not sure if this is necessary
        let isUserInterested = false // we want to assume this starts false.. if an activity is found we can overwrite it

        selectedActivityCopy.user_activities.forEach(userActivity => {
            if (userActivity.user_id == user.id) { // find if the user_activity exists for current user and if so get the value from there
            userActivity.interested = !userActivity.interested
            isUserInterested = userActivity.interested
        }
    })
...
            If we look at the accepted activity and it has a previous user_activity it all works great. but the if the for loop
